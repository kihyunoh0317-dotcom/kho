"""
한국은행 ECOS Open API 국고채(10년) 데이터 수집 스크립트
- 통계표: 817Y002 (시장금리(일별))
- 통계항목: 010210000 (국고채(10년))
- 환경변수: .env 파일의 ECOS_API_KEY
- 출력: 날짜(TIME), 값(DATA_VALUE) 컬럼을 가진 CSV 파일 (기본값: ecos_ktb_10y.csv)
"""
...
DEFAULT_ITEM_CODE = "010210000"     # 국고채(10년)
DEFAULT_CYCLE = "D"                 # 일별 (D)
DEFAULT_YEARS = 5                   # 최근 5개년
DEFAULT_OUTPUT_CSV = "ecos_ktb_10y.csv"
...
def fetch_ecos_data(
    ...
    print(f" - 통계항목 코드: {item_code} (국고채 10년)")
    print(f" - 조회 주기: {cycle}")
...
def main():
    parser = argparse.ArgumentParser(description="한국은행 ECOS API 국고채(10년) 데이터 수집기")
    parser.add_argument(
        "--api-key",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        help=f"통계항목 코드 (기본값: {DEFAULT_ITEM_CODE} - 국고채 10년)"
    )
