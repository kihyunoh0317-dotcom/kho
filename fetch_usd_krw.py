"""
한국은행 ECOS Open API 원/달러 환율(매매기준율) 데이터 수집 스크립트
- 통계표: 731Y001 (주요국 통화의 대원화환율)
- 통계항목: 0000001 (원/미국달러(매매기준율))
- 환경변수: .env 파일의 ECOS_API_KEY
- 출력: 날짜(TIME), 값(DATA_VALUE) 컬럼을 가진 CSV 파일 (기본값: usd_krw.csv)
"""
...
# 기본 설정값
DEFAULT_STAT_CODE = "731Y001"       # 3.1.1.1. 주요국 통화의 대원화환율
DEFAULT_ITEM_CODE = "0000001"       # 원/미국달러(매매기준율)
DEFAULT_CYCLE = "D"                 # 일별 (D)
DEFAULT_YEARS = 5                   # 최근 5개년
DEFAULT_OUTPUT_CSV = "usd_krw.csv"
...
def fetch_ecos_data(
    ...
    print("[정보] ECOS API 환율 데이터 수집 시작...")
    print(f" - 통계표 코드: {stat_code} (주요국 통화의 대원화환율)")
    print(f" - 통계항목 코드: {item_code} (원/미국달러(매매기준율))")
    print(f" - 조회 주기: {cycle}")
...
def main():
    parser = argparse.ArgumentParser(description="한국은행 ECOS API 원/달러 환율(매매기준율) 데이터 수집기")
    parser.add_argument(
        "--api-key",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        help=f"통계항목 코드 (기본값: {DEFAULT_ITEM_CODE} - 원/미국달러(매매기준율))"
    )
